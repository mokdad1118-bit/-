...
### Web Push (إشعارات عند إغلاق التطبيق)

- على Render: **`VAPID_PUBLIC_KEY`** و **`VAPID_PRIVATE_KEY`** (و **`VAPID_CONTACT_EMAIL`** اختياري)، و **`PUBLIC_URL`** = رابط موقع Netlify (لأيقونات الإشعار).
- ملف **`/sw.js`** يجب أن يُحمَّل من **نطاق Netlify** (نفس أصل PWA). Render يحفظ اشتراك المستخدم في قاعدة البيانات ويرسل Push؛ **لا يكفي** فتح واجهة Render فقط بدون Netlify.
- **`CORS_ORIGIN`** على Render يجب أن يتضمن رابط Netlify حتى يعمل جلب مفتاح VAPID من المتصفح.
- من لوحة التحكم → تبويب الإشعارات → **فحص إعداد Push**: عدد الاشتراكات المحفوظة وحالة VAPID.
