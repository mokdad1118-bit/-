...
5. **Cloudinary (رفع الصور):** من [Cloudinary Dashboard](https://cloudinary.com/console) → **Settings → API Keys** — انسخ **`CLOUDINARY_URL`** (أو `CLOUDINARY_CLOUD_NAME` + `CLOUDINARY_API_KEY` + `CLOUDINARY_API_SECRET`) وأضفها في Render → **Environment** للخدمة، ثم **Manual Deploy**. بدونها لن يعمل رفع الصور من `/admin`.
...
5. **Environment → Add Environment Variable:**
   ...
   | `CLOUDINARY_URL` | من Cloudinary → API Keys (أو الثلاثة `CLOUDINARY_*` بدل السطر) — **مطلوب لرفع الصور** |
