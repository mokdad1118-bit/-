...
### الصور على Render (مُوصى به: Cloudinary)

**الأفضل:** فعّل **Cloudinary** في متغيرات البيئة (انظر `.env.example`). عندها الرفع يُخزَّن كرابط HTTPS فقط في قاعدة البيانات ولا يُحذف مع إعادة النشر.

**بديل (قرص دائم):** إذا لم تستخدم Cloudinary، صور `/uploads` على القرص المؤقت قد تختفي بعد كل إعادة نشر:

1. من خدمة الويب في Render: **Disks → Add Disk**
2. Mount Path: `/var/data/uploads`
3. من **Environment** أضف: `UPLOADS_DIR=/var/data/uploads`
4. أعد النشر.
